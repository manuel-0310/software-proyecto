...
def obtener_invitado(invitado_id: int):
    ...
    if not invitado:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Invitado no encontrado")
    return invitado


@router.post("/", response_model=InvitadoResponse, status_code=status.HTTP_201_CREATED)
def crear_invitado(invitado: InvitadoCreate, _: str = Depends(get_current_user)):
