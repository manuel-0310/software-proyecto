...
class _PresupuestoScreenState extends State<PresupuestoScreen> {
  ...
  void initState() {
    super.initState();
    // TODO(JWT): recalcular() hace GET /presupuesto/ (público, sin token).
    controller.recalcular().then((_) { if (mounted) setState(() {}); });
  }
...
// Variable para cerca.
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async { await controller.recalcular(); if (mounted) setState(() {}); },
        child: ListView(
          padding: const EdgeInsets.all(16),
          children: [
            // Banner de Circuit Breaker: visible cuando el backend no responde.
            if (controller.circuitoAbierto)
              _CircuitBreakerBanner(
                mensaje: controller.mensajeError ??
                    'Servicio no disponible. Mostrando datos en caché.',
                onReintentar: () async {
...
/// Banner de resiliencia: aparece cuando el Circuit Breaker está OPEN.
class _CircuitBreakerBanner extends StatelessWidget {
