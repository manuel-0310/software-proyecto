...
// Variable para invitado.
  ...
  /// Reemplaza la caché local con la lista recibida desde el backend remoto.
  /// Llamado por [InvitadoController] tras sincronizar con la Weddy API.
  void sincronizar(List<Invitado> lista) {
    for (final inv in _repository.obtenerTodos().toList()) {
      _repository.eliminar(inv.id);
    }
    for (final inv in lista) {
      for (final obs in _observadoresGlobales) {
        inv.suscribir(obs);
      }
      _repository.guardar(inv);
    }
  }
